        ...
        where TUser : LthUser, new()
    {
            ...
            ([FromBody] RegisterDTO registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            if (string.IsNullOrEmpty(confirmPassword) || !confirmPassword.Equals(password) )
            {
                ...
                identityError.Code = "BadConfirm";
                identityError.Description = "Confirm Password is empty or does not match the password.";
                return CreateValidationProblem(IdentityResult.Failed(identityError));
            }
